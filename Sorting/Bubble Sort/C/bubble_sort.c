...
void bubble_sort(int v[]){
	int i,k,aux = 0;
	for(i = 0; i < n; i++){
		for(k = 0; k < n-1-i; k++){    // i is beign minus because every time we run through 1 iteration our biggest element is at last index so we need not to compare that again
